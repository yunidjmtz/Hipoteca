...
test('Mi plan hipotecario reúne el desembolso y la meta de ahorro', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Ver escala de precios →' })).toBeVisible();
  ...
  await expect(page.getByText('Capacidad de compra estimada', { exact: true })).toHaveCount(0);
